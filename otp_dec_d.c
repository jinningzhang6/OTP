...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		charsWritten = send(establishedConnectionFD, dep, strlen(dep), 0); // Write to the Client
		if (charsWritten < 0) error("Server: ERROR writing to socket");
		if (charsWritten < strlen(dep)) printf("Server: WARNING: Not all data written to socket!\n");
...
void decrypt(char *msg1, char *msg2, char *final) {
	int i, j, k, m;
	char x;
